...
import styles from './HideSelections.module.css';
import { pb } from '@/lib/pb.client';
import { updatePlayer } from '@/actions/players';

export interface HideSelectionsProps {
  playerId: string;
}
export const HideSelections = ({ playerId }: HideSelectionsProps) => {
  const [hide, setHide] = useState(true);

  const handleClick = () => {
    updatePlayer(playerId, { hideSelections: !hide });
  };
  ...
  const fetchSetting = async () => {
    ...
    if (hideSelections === undefined) return;

    setHide(hideSelections);
  }

  useEffect(() => {
    fetchSetting();
    pb.collection('players').subscribe('*', fetchSetting);
    return () => {
  ...
  return (
    <>
      {hide ? (
        <button className={styles.eye} onClick={handleClick}>
